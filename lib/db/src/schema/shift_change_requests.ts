...
import { shiftTemplatesTable } from "./shifts";

export const shiftChangeStatusEnum = pgEnum("shift_change_status", [
  "Pending",
  "Approved",
  "Rejected",
  "Cancelled",
...
export const shiftChangeRequestsTable = pgTable("shift_change_requests", {
  ...
  currentShiftId: integer("current_shift_id").references(() => shiftTemplatesTable.id),
  requestedShiftId: integer("requested_shift_id").notNull().references(() => shiftTemplatesTable.id),
  effectiveDate: date("effective_date").notNull(),
  reason: text("reason").notNull(),
  status: shiftChangeStatusEnum("status").notNull().default("Pending"),
